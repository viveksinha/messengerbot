...
/**
 * Main Servlet for FaceBot framework. This servlet requires an init-param containing the fully
 * qualified name of a class implementing {@link FaceBotDefinition} in which the initial
 * configuration is done. If not such class is found or can't be loaded, a ServletException is
 * thrown during initialization.
 *
 * The FaceBotServlet supports GET requests only for the Subscribing phase and POST requests for all
 * the Facebook callbacks. For more information about how the communication is handled, check the
 * documentation for {@link #doGet}, {@link #doPost(HttpServletRequest, HttpServletResponse)} and
 * Facebook's documentation with the link below.
 *
 * @author Donato
 * @date 31/lug/2016
 * @see <a href="https://developers.facebook.com/docs/messenger-platform/quickstart"> Facebook
 * Subscription info</a>
 */
public class FaceBotServlet extends HttpServlet {
    ...
    /**
     * Initializes the FaceBotServlet. The initialization will try to load the class implementing
     * {@link FaceBotDefinition} passed has an init-param in the web.xml. If the class is found and
     * correctly loaded, the method {@link FaceBotDefinition#defineBehavior()} is called, otherwise
     * a ServletException is thrown.
     *
     * @throws ServletException if the FaceBotDefinition class is not found in the servlet
    ...
    /**
     * Specifies how to handle a GET request. GET requests are used by Facebook only during the
     * WebHook registration. During this phase, the FaceBotServlet checks that the {@link
     * FaceBotConstants#HUB_MODE_PARAMETER} value received equals to {@link
     * FaceBotConstants#HUB_MODE_SUBSCRIBE} and that the {@link FaceBotConstants#HUB_VERIFY_TOKEN_PARAMETER}
     * value received equals to the {@link FaceBotContext#getValidationToken()}. If that's true,
     * then the FaceBotServlet will reply sending back the value of the {@link
     * FaceBotConstants#HUB_CHALLENGE_PARAMETER} received, in order to confirm the registration,
     * otherwise it will return an error 403.
     */
    ...
    /**
     * Specifies how to handle a POST request. It parses the request as a {@link MessengerCallback}
     * object. If the request is not a MessengerCallback, then the FaceBotServlet logs an error and
     * does nothing, otherwise it will forward the request to all registered FaceBots in order to
     * let them process the callbacks.
     */
    @Override
    ...
    /**
     * Method which returns the first String in a String array if present or the empty String
     * otherwise. Used to unwrap the GET arguments from an {@link HttpServletRequest#getParameterMap()}
     * which returns a String array for each GET parameter.
     *
     * @param parameter the String array to unwrap.
     * @return the first String of the array if found or the empty String otherwise.
     */
